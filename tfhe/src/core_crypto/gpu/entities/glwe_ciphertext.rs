...
use crate::core_crypto::prelude::{
    ...
    GlweDimension, PolynomialSize, UnsignedInteger,
};

/// A structure representing a GLWE ciphertext with 64 bits of precision on the GPU.
#[derive(Debug)]
pub struct CudaGlweCiphertext<T: UnsignedInteger>(pub(crate) CudaGlweList<T>);
...
impl<T: UnsignedInteger> CudaGlweCiphertext<T> {
    pub fn new(
        ...
        let cuda_glwe_list = CudaGlweList {
            d_vec,
            glwe_ciphertext_count: GlweCiphertextCount(1),
            glwe_dimension,
            polynomial_size,
            ciphertext_modulus,
        };
    ...
    pub(crate) fn to_glwe_ciphertext(&self, streams: &CudaStreams) -> GlweCiphertext<Vec<T>> {
        let glwe_ct_size =
        ...
        GlweCiphertext::from_container(container, self.polynomial_size(), self.ciphertext_modulus())
    }
